...
2. **Data Fetching and Processing**:
   - Fetch historical stock prices (can extend to use APIs).
...
try:
    ...
    data = pd.read_csv('data/stock_prices.csv', index_col=0, parse_dates=True)  #Replace with API call if needed
    st.write(data.head())
...
if st.sidebar.button("Run Optimization"):
    ...
    st.write({tickers_list[i]: round(optimal_weights[i], 4) for i in range(len(tickers_list))})
    ...
    plot_allocation(optimal_weights, tickers_list)
